...
if __name__ == '__main__':
    ...
    if args.simple:
        ...
        output_files = {key: open(f"shannon_output_{key}.jsonl", "w", encoding="utf-8") for key in args.summ_keys}
        ...
        for record in tqdm(selection):
            for summ_key in args.summ_keys:
                ...
                if args.measure_t or args.measure_summ:
                    ...
                    ll_base, ll_help, ll_full, S, num_doc_tokens, num_summ_tokens = s.go(record['doc'], summary_text)
                    output_data = {
                        'doc_id': record['id'],
                        'system': record['model_id'],
                        'summary_type': summ_key,
...
# test: python shannon.py --simple
# run on m22 data: python shannon.py --input_file ../data/merged_adversarial_m22.jsonl --summ_keys decoded