...
def load(input_file, output_file):
    ...
    df = pd.DataFrame.from_records(result_blobs)
    ...
    df['shannon_star'] = 2 * df['shannon_score'] * (1 - df['compression']) / (df['shannon_score'] + (1 - df['compression']))
...
if __name__ == "__main__":
    adversarial_sum = ["decoded", "decoded_2x_expanded", "decoded_3x_expanded", "reordered", "decoded_add_lead3", "decoded_add_rand3", "antonym_substituted_2", "synonym_substituted_2"]
    input_files = [f"shannon_output_{sum}.jsonl" for sum in adversarial_sum]
    for file in input_files:
        output_file = f"../util_data/shannon/score_{file}"
        load("../util_data/"+file, output_file)

# https://github.com/PrimerAI/blanc/blob/master/shannon/summeval_score.py