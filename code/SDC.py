...
class Shannon:
    ...
    def go(self, doc, summ, measure_t=False, measure_summ=False):
        ...
        if measure_t:
            ...
            return token_lls_base, token_lls_help, token_lls_full, S, len(doc_tokens), len(summ_tokens)

    def log(self, s=None):
        if self.verbose:
            print(s)

    def sdc(self, doc, summ):
        dt_base, dt_help, dt_full, S, num_doc_tokens, num_summ_tokens  = self.go(doc, summ)
        p,_ = scipy.stats.pearsonr(dt_help, dt_base)
        W_DS = sum(dt_help)-sum(dt_base)
        W_DS_norm = 1 / (1 + np.exp(-W_DS))
        P_D = np.exp(dt_base)
        P_D_given_S = np.exp(dt_help)
        pearson2, _ = scipy.stats.pearsonr(P_D, P_D_given_S)
        SDC = W_DS_norm * p
        CR = num_summ_tokens / num_doc_tokens
        SDC_star = (2 * SDC * (1 - CR)) / (SDC + (1 - CR))

        print("SDC: ", SDC)
        print("SDC_star: ", SDC_star)
        return SDC, SDC_star
...
if __name__ == '__main__':
    ...
    parser.add_argument('--summ_keys', type=str, nargs='+', default=['decoded'])
    ...
    if args.simple:
        ...
        output_files = {key: open(f"score_sdc_output_{key}.jsonl", "w", encoding="utf-8") for key in args.summ_keys}

        for record in tqdm(selection):
            for summ_key in args.summ_keys:
                if summ_key not in record:
                    print(f"⚠️ Warning: {summ_key} not in record {record['id']} ")
                    continue

                summary_text = record[summ_key]

                if args.measure_t or args.measure_summ:
                    ll = s.go(record['doc'], summary_text, measure_summ=args.measure_summ, measure_t=args.measure_t)
                    output_data = {
                        'doc_id': record['id'],
                        'system': record['model_id'],
                        'summary_type': summ_key,
                        'll_summ': ll,
                    }
                else:
                    sdc, sdc_star = s.sdc(record['doc'], summary_text)
                    output_data = {
                        'doc_id': record['id'],
                        'system': record['model_id'],
                        'summary_type': summ_key,
                ...
                output_files[summ_key].write(json.dumps(output_data, ensure_ascii=False) + "\n")
